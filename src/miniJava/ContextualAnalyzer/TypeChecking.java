...
public class TypeChecking implements Visitor<Object, TypeDenoter> {
    ...
    public TypeDenoter visitVardeclStmt(VarDeclStmt stmt, Object arg) {
		...
		if (!typeEquals(refType, exprType)) {
            if (refType instanceof ClassType && exprType instanceof ClassType) {
                ...
                reporter.reportError("*** line " + stmt.varDecl.posn.lineNumber + ": Type error - "
                        + " attempts to assign expression of type " + exprType.typeKind
    ...
    case MINUS:
        if (!typeKindEquals(expr.expr.visit(this, null), TypeKind.INT)) {
            reporter.reportError("*** line " + expr.posn.lineNumber + ": Type error - "
                   + "operator " + expr.operator.kind + " is not of type INT.");
    ...
    case NOT:
        if (!typeKindEquals(expr.expr.visit(this, null), TypeKind.BOOLEAN)) {
            reporter.reportError("*** line " + expr.posn.lineNumber + ": Type error - "
                   + " operator " + expr.operator.kind + " is not of type BOOLEAN.");
            return new BaseType(TypeKind.ERROR, expr.posn);
        }
...
*/
    ...
    /*public boolean checkTypeKind(TypeDenoter x, TypeKind y) {
	if(typeKindEquals(x,y)) {
		reporter.reportError( "*** line " + x.posn.lineNumber + ": Type error - "
                            + "operator "+ " is not of type INT.");
		return false;
	}  	
	return true;
}*/
    ...
    public TypeDenoter visitBinaryExpr(BinaryExpr expr, Object arg) {
        ...
        case LESS:
            if (!typeKindEquals(leftType, TypeKind.INT)) {
                reporter.reportError(
                        "*** line " + expr.left.posn.lineNumber + ": Type error - "
                                + "operator " + expr.operator.kind + " is not of type INT.");
                return new BaseType(TypeKind.ERROR, expr.posn);
            }
            if (!typeKindEquals(rightType, TypeKind.INT)) {
                reporter.reportError(
                        "*** line " + expr.right.posn.lineNumber + ": Type error - "
                                + "operator " + expr.operator.kind + " is not of type INT.");
                return new BaseType(TypeKind.ERROR, expr.posn);
            }
            combinedType = new BaseType(TypeKind.BOOLEAN, expr.posn);
        ...
        case NOTEQUAL:           
            if (!typeEquals(leftType, rightType)) {
				if (leftType instanceof ClassType && rightType instanceof ClassType) {
					...
					reporter.reportError("*** line " + expr.posn.lineNumber + ": Type error - "+ " binary expression has unmatched types " + className1.decl.type.typeKind + " and "
							+ className2.decl.type.typeKind + ".");
				} else {
					reporter.reportError("*** line " + expr.posn.lineNumber + ": Type error - "
							+ "binary expression has unmatched types " + leftType.typeKind + " and "
							+ rightType.typeKind + ".");
				}
                return new BaseType(TypeKind.ERROR, expr.posn);
            }
