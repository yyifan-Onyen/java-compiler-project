	...
	private ClassDecl parseClassDecl() throws SyntaxError {
		...
		while (token.kind!= TokenKind.RBRACE) {
			...
			if(token.kind == TokenKind.VOID) {
				...
				MemberDecl thisField = new FieldDecl(isPrivate, isStatic, thisType, methodName, currentPosition);
        ...
        case THIS:
            ...
            if (token.kind == TokenKind.LPAREN) { // this (.id)* "(" ArgumentList? ")"
                ...
                if (token.kind == TokenKind.LBRACKET) {
                    ...
                    accept(TokenKind.ASSIGN);
                    variableExpression = parseExpression();
                    thisStatement = new IxAssignStmt(thisReference, ixExpression, variableExpression, statementStart);
                }
                else {
                accept(TokenKind.ASSIGN);
                variableExpression = parseExpression();
